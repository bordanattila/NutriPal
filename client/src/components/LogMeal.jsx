 ...
 * @file LogMeal.jsx
 * @module LogMeal
 * @description Allows users to view a meal's nutrition details and log it to their Daily Log.
 */
...
/**
 * FoodDetails component displays the meal information and allows logging it to Daily Log.
 * @returns {JSX.Element}
 */
const FoodDetails = () => {
  const { mealName, mealID } = useParams();
  ...
  useEffect(() => {
    const fetchRecipeDetails = async () => {
      try {
        const response = await api.get(`api/log-meal/${mealID}?servings=${servingCount + fractionValue}`);
    ...
    fetchRecipeDetails();
  }, [mealID, servingCount, fractionValue, servingID]);
