...
export default function RecipeFoodDetailsScreen() {
  ...
  const handleAddIngredient = async () => {
    if (!selectedServing) {
      setError('Please select serving size');
      return;
    }
    ...
    try {
      ...
      const currentIngredients = storedIngredients ? JSON.parse(storedIngredients) : [];
      const currentIngredientsID = storedIngredientsID ? JSON.parse(storedIngredientsID) : [];

      // Add new ingredient (without creating database entry yet)
      const newIngredient = {
        name: foodDetails?.food_name || '',
        servingCount: servingCount,
        servingSize: selectedServing.serving_description,
        food_id: foodDetails?.food_id || '',
        // Store nutrition data for recipe calculation
      ...
      // For now, just store the ingredient data locally
      // The actual database entries will be created when the recipe is saved
      const updatedIngredientsID = [...currentIngredientsID, `temp_${Date.now()}`];

      // Save to storage
      await SecureStore.setItemAsync('ingredientsList', JSON.stringify(updatedIngredients));
      await SecureStore.setItemAsync('ingredientsID', JSON.stringify(updatedIngredientsID));
